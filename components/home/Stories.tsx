...
export default function Stories() {
  ...
  return (
    <section className="bg-white max-w-6xl mx-auto px-3 sm:px-4 py-10 sm:py-20">
      <span className="uppercase text-xs tracking-widest text-gray-500 font-semibold mb-2 block">Stories</span>
      <h2 className="text-2xl xs:text-3xl sm:text-4xl md:text-5xl font-bold text-gray-900 mb-3 sm:mb-4 leading-tight">Guest experiences.<br />In their own words.</h2>
      <p className="text-gray-700 mb-8 sm:mb-12 max-w-xs xs:max-w-md sm:max-w-2xl text-base sm:text-lg">See what our guests have to say about their stay at LORD Resort. We take pride in creating memorable experiences for everyone who visits.</p>
      {/* Progress bar */}
      <div className="flex justify-center gap-1 xs:gap-2 sm:gap-3 mb-6 sm:mb-8">
        {reviews.map((_, i) => (
          <button
            key={i}
            onClick={() => setCurrent(i)}
            aria-label={`Go to review ${i + 1}`}
            className={`h-2 w-6 xs:w-8 sm:w-12 rounded transition-all duration-300 focus:outline-none ${i === current ? "bg-gray-900" : "bg-gray-200"}`}
            type="button"
          />
        ))}
      </div>
      ...
      <div className="flex justify-center">
        <div className={`bg-gray-50 rounded-2xl shadow p-4 xs:p-6 flex flex-col h-full max-w-xs xs:max-w-md sm:max-w-xl w-full transition-opacity duration-300 ${fade ? "opacity-100" : "opacity-0"}`}>
          <div className="mb-3 xs:mb-4">
            <svg className="w-7 h-7 xs:w-8 xs:h-8 text-[#0072bc] mb-1 xs:mb-2" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M9 7h.01M15 7h.01M7 11c0 2.21 1.79 4 4 4s4-1.79 4-4V5a2 2 0 00-2-2H9a2 2 0 00-2 2v6z" /></svg>
            <p className="text-gray-800 text-sm xs:text-base mb-1 xs:mb-2">{reviews[current].text}</p>
          </div>
          <div className="mt-auto flex items-center gap-2 xs:gap-3">
            <div className="flex flex-col">
              <span className="font-semibold text-gray-900 text-sm xs:text-base">{reviews[current].name}</span>
            ...
            <div className="ml-auto flex items-center gap-1">
              <span className="text-yellow-400 font-bold text-sm xs:text-base">{reviews[current].rating}</span>
              <svg className="w-3.5 h-3.5 xs:w-4 xs:h-4 text-yellow-400" fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.286 3.967a1 1 0 00.95.69h4.175c.969 0 1.371 1.24.588 1.81l-3.38 2.455a1 1 0 00-.364 1.118l1.287 3.966c.3.922-.755 1.688-1.54 1.118l-3.38-2.454a1 1 0 00-1.175 0l-3.38 2.454c-.784.57-1.838-.196-1.54-1.118l1.287-3.966a1 1 0 00-.364-1.118L2.05 9.394c-.783-.57-.38-1.81.588-1.81h4.175a1 1 0 00.95-.69l1.286-3.967z" /></svg>
            </div>
          </div>
        </div>
      </div>
