    ...
    <section className="bg-gray-50 py-20 flex justify-center items-center">
      <div className="max-w-7xl w-full mx-auto px-6 py-12">
        <div className="max-w-4xl mx-auto text-center mb-12">
          ...
          <h2 className="text-4xl md:text-5xl font-bold text-gray-900 mb-2 leading-tight">The Principles That</h2>
          <h3 className="text-2xl font-light text-gray-700 mb-6">Guide Our Journey</h3>
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-8 mb-12">
          {values.map((v) => (
            <div
              key={v.title}
              className="group bg-white rounded-2xl shadow p-8 flex flex-col items-center text-center relative overflow-hidden cursor-pointer"
            >
              {/* Hover image */}
              ...
              <div className="mb-4 z-10 relative">{v.icon}</div>
              <div className="font-semibold text-lg text-gray-900 mb-2 z-10 relative">{v.title}</div>
              <div className="text-gray-600 text-sm z-10 relative">{v.desc}</div>
              {/* Decorative background shape */}
              <div className="absolute bottom-0 right-0 w-24 h-24 bg-blue-50 rounded-full opacity-40 -z-10" />
            </div>
          ))}
        </div>
