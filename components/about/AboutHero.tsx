      ...
      <div className="relative z-20 w-full max-w-7xl mx-auto px-3 xs:px-4 sm:px-6 flex flex-col justify-center h-full pt-16 xs:pt-20 sm:pt-24">
        ...
        <div className="flex gap-3 xs:gap-4 overflow-x-auto pb-2 scrollbar-thin scrollbar-thumb-gray-300">
          {VENUES.map((venue, i) => (
            <motion.div
              key={venue.title}
              className="rounded-xl overflow-hidden shadow-lg bg-white/10 backdrop-blur-sm w-40 min-w-[150px] xs:w-48 xs:min-w-[180px] cursor-pointer group flex-shrink-0"
            ...
            >
              ...
              <div className="p-2 xs:p-3 text-white text-xs xs:text-sm font-medium bg-black/40 text-center">{venue.title}</div>
            </motion.div>
          ))}
        </div>
      </div>
